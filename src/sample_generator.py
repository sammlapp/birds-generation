...
# notes on API:
# model.encode(img) returns (quant_top, quant_bottom, diff, id_top, id_bottom)
# model.decode(quant_top,quant_bottom) returns the reconstructed image
# model.to_audio_signal(model.decode(sample))
# model = VQVAE(in_channel=1) to initialize default model, or
# model = load_model(model, model_path, device='cpu') to load a model from a path
...
class SampleGenerator():
    ...
    def encode(self, sample):
        ...
        Args: 
            - sample: torch.tensor eg torch.tensor(self.load_sample(path))
